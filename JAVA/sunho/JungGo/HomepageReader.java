...
public class HomepageReader {
	...
	public void homepageReading() {// 네이버에 접속해서 글 읽어오기
			...
			public void run() {
				while (true) {
					...
					try {
						...
						for (int i = 0; i < lines_list.size(); i++) {// line단위의소스 하나의String으로 합치기
							fullContents += lines_list.get(i);
						}

						first_arr = fullContents.split("list_wrap");// 하나의 String을 split하기_1
